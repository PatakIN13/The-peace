...
function updateDepth(book, newPage) {
	...
	if (newPage>3)
		$('.sj-book-phone .p2 .depth').css({
			width: depthWidth,
			left: 20 - depthWidth
		});
	else
		$('.sj-book-phone .p2 .depth').css({width: 0});

		depthWidth = 16*Math.min(1, (pages-page)*2/pages);

	if (newPage<pages-3)
		$('.sj-book-phone .p111 .depth').css({
			width: depthWidth,
			right: 20 - depthWidth
		});
	else
		$('.sj-book-phone .p111 .depth').css({width: 0});
...
function loadPage(page) {
	...
	$.ajax({url: 'pages/page' + page + '.html'}).
			...
			$('.sj-book-phone .p' + page).html(pageHtml.replace('samples/steve-jobs/', ''));
		});
...
function zoomThis(pic) {
	...
	var	position, translate,
		...
		tmpPic = $('<img />'),
		zCenterX = $('#book-zoom-phone').width()/2,
		zCenterY = $('#book-zoom-phone').height()/2,
		bookPos = $('#book-zoom-phone').offset(),
		picPos = {
			left: pic.offset().left - bookPos.left,
			top: pic.offset().top - bookPos.top
		},
		completeTransition = function() {
			$('#book-zoom-phone').unbind(transitionEnd);
		...
		$('.sj-book-phone').data().zoomIn = true;

		$('.sj-book-phone').turn('disable', true);
		...
		tmpPic.load(function() {
			...
			$('#book-zoom-phone').transform(
				'translate('+translate.left+'px, '+translate.top+'px)' +
				'scale('+zoomFactor+', '+zoomFactor+')');

			if (transitionEnd)
				$('#book-zoom-phone').bind(transitionEnd, completeTransition);
			else
...
function zoomOut() {
	...
	var transitionEnd = $.cssTransitionEnd(),
		completeTransition = function(e) {
			$('#book-zoom-phone').unbind(transitionEnd);
			$('.sj-book-phone').turn('disable', false);
			$('body').css({'overflow': 'auto'});
			moveBar(false);
		};

	$('.sj-book-phone').data().zoomIn = false;
	...
	$('.zoom-pic').remove();
	$('#book-zoom-phone').transform('scale(1, 1)');
